...
kable(head(df_clus,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "285px")
```  
...
df_clus$GRAVEDAD <- ifelse(df_clus$GRAVEDAD == "SOLO DAÑOS", "SOLO_DANOS", df_clus$GRAVEDAD) 
...
```{r}
library(factoextra) # extensión basada en ggplot2 para crear visualizaciones de los resultados de clustering y su evaluación.
library(cluster)
library(tidyr) # Librería para transformar un DataFrame
```  


```{r}

# Se crean las diferentes métricas que serán utilizadas para hacer el clustering
...
# Se crea la agrupación barrio - gravidad
...
kable(head(clust1,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "215px")
...
# Se crea la agrupación barrio - fechas especiales
clust2 <-  cluster%>%
  group_by(BARRIO,  FECHAS_ESP)%>%
  summarise(
    total = n()
    )
...
# Se crea la agrupación barrios - periodos
clust3 <-  cluster%>%
...
# Se crea la agrupacion barrio - clase (choque, atropello, caida ocupante, incendio, volcamiento, otro)
clust4 <-  cluster%>%
...
clust1 <- spread(data = clust1, key = GRAVEDAD, total)
clust2 <- spread(data = clust2, key = FECHAS_ESP, total)
clust3 <- spread(data = clust3, key = PERIODO, total)
clust4 <- spread(data = clust4, key = CLASE, total)
...
cluster<-full_join(clust1,clust2,by="BARRIO")
cluster<-full_join(cluster,clust3,by="BARRIO")
cluster<-full_join(cluster,clust4,by="BARRIO")

kable(head(cluster,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "280px")
```  
...
Se crea el primer indicador que establece la proporción de accidentes los fines de semana y festivos vs el total de accidentes: 
(suma de accidentes en dias festivos y no laborales)/ suma total de accidentes.  

```{r}

# Porcentaje de accidentes fines de semana y festivos: accidentes fines de semana y festivos dividido sobre el total de accidentes

cluster$Porcentaje_accidentes_finsemyfestivos <- (cluster$`NO LABORAL`+cluster$FESTIVO)/(cluster$FESTIVO+cluster$LABORAL+cluster$`NO LABORAL`)

```  

El segundo indicador establece la proporción entre el número de accidentes fatales vs el total de accidentes:
suma de accidentes faltales / suma total de accidentes.  

```{r}

# Porcentaje de fatalidad: accidentes fatales dividido por el total de accidentes

cluster$Porcentaje_fatalidad <- (cluster$MUERTO)/(cluster$HERIDO+cluster$MUERTO+cluster$SOLO_DANOS)


```  

El tercer indicador permite conocer el número de accidentes promedio por año:
suma de los accidentes de todos los años / nro de años.  

```{r}

# Promedio anual de accidentes por barrio (2014-2018)

cluster$promedio_anual_accidentes <- (cluster$`2014`+cluster$`2015`+cluster$`2016`+cluster$`2017`+cluster$`2018`)/5


```  

El ultimo indicador permite establecer que porcentaje de los accidentes que se presentaron fueron atropellos:
suma de accidentes con atropellos / suma total de accidentes.  

```{r}

# Tasa de atropello

cluster$Porcentaje_atropello <- (cluster$ATROPELLO)/(cluster$ATROPELLO+cluster$`CAIDA OCUPANTE`+cluster$CHOQUE+cluster$OTRO+cluster$VOLCAMIENTO)
...
cluster<-subset(cluster, select = c(BARRIO,Porcentaje_accidentes_finsemyfestivos,Porcentaje_fatalidad,promedio_anual_accidentes,Porcentaje_atropello) )
...
# Se pone el barrio como nombre de las columnas y luego se elimina del dataframe

cluster <- data.frame(cluster)
rownames(cluster) <- unlist(cluster[,1])
...
# Se escalan el dataframe para evitar problemas en k-means por diferencia de escalas

cluster_sc<-scale(cluster)

kable(head(cluster_sc,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "215px")
```  
...
# Según el método del codo, la varianza intraclusters deja de ser significativa a partir de k=5
...
wcss <- vector()
for(i in 1:20){
  wcss[i] <- sum(kmeans(cluster_sc,i)$withinss)
}
...
fviz_nbclust(cluster_sc, kmeans, method = "silhouette")
```  
...
# Método clusgap para evaluar el número de clusters

gapstat <- clusGap(cluster_sc, FUN = kmeans, B =250, nstart = 40,
                    K.max = 10 )
...
```{r}
# Se prueban varios clusters variando k

cluster2 <- kmeans(cluster_sc, centers = 2, iter.max=2000, nstart=100)
cluster3 <- kmeans(cluster_sc, centers = 3, iter.max=2000, nstart=100)
cluster4 <- kmeans(cluster_sc, centers = 4, iter.max=2000, nstart=100)
cluster5 <- kmeans(cluster_sc, centers = 5, iter.max=2000, nstart=100)
cluster6 <- kmeans(cluster_sc, centers = 6, iter.max=2000, nstart=100)
cluster7 <- kmeans(cluster_sc, centers = 7, iter.max=2000, nstart=100)
cluster8 <- kmeans(cluster_sc, centers = 8, iter.max=2000, nstart=100)
cluster9 <- kmeans(cluster_sc, centers = 9, iter.max=2000, nstart=100)
cluster10 <- kmeans(cluster_sc, centers = 10, iter.max=2000, nstart=100)
```  
...
# Se genera un dataframe con las agrupaciones con diferentes k
...
clust.mat <- cbind(clust.mat, cluster2$cluster,cluster3$cluster, cluster4$cluster,cluster5$cluster,cluster6$cluster,cluster7$cluster,cluster8$cluster,cluster9$cluster,cluster10$cluster)


colnames(clust.mat) <- lapply(c("Cl1","Cl2","Cl3","Cl4","Cl5","Cl6","Cl7","Cl8","Cl9","C10"),paste,"G", sep="")


clust.mat <- as.data.frame(clust.mat)

kable(head(clust.mat,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "210px")
...
# Se define en una variable el tamaño de los grupos

tamano_grupo<-cluster5$size
tamano_grupo
...
# Se muestran las medias de la agrupación con k=5

media<-aggregate(cbind(Porcentaje_accidentes_finsemyfestivos,Porcentaje_fatalidad,promedio_anual_accidentes,Porcentaje_atropello) ~ cluster5$cluster, data=cluster, FUN=mean)
...
# Se fusiona tamano_grupo con media


kable(cbind(tamano_grupo, media)) %>% 
  kable_paper() %>%
...
# Se muestran las desviaciones de la agrupación con k=5

desv <- aggregate(cbind(Porcentaje_accidentes_finsemyfestivos,Porcentaje_fatalidad,promedio_anual_accidentes,Porcentaje_atropello) ~ cluster5$cluster, data=cluster, FUN=sd)
...
# Se fusiona tamano_grupo con desv 


kable(cbind(tamano_grupo, desv)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "200px")
```  
...
df1 = cbind("BARRIO"=rownames(cluster),cluster)

df2 = cbind("BARRIO"=rownames(clust.mat),clust.mat)

cluster_final<-merge(df1,df2,by="BARRIO",all=T)

cluster_final<-subset(cluster_final, select = c(BARRIO,Porcentaje_accidentes_finsemyfestivos,Porcentaje_fatalidad,promedio_anual_accidentes,Porcentaje_atropello,Cl5G) )


kable(head(cluster_final,5)) %>% 
  kable_paper() %>%
    scroll_box(width = "850px", height = "200px")
```  
...
# Se carga el dataframe shp (shape)

library(sf)
shp <- read_sf("shp/Limite_Barrio_Vereda_Catastral.shp")
```  
...
med <- subset(x = shp, subset = COMUNA != 50)
med <- subset(x = med, subset = COMUNA != 60)
med <- subset(x = med, subset = COMUNA != 70)
med <- subset(x = med, subset = COMUNA != 80)
med <- subset(x = med, subset = COMUNA != 90)
```  
...
med$NOMBRE_BAR <- gsub("Aures No.1", "AURES NO. 1", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("El Progreso No.2", "EL PROGRESO NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Las Lomas No.1", "LAS LOMAS NO. 1", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Las Lomas No.2", "LAS LOMAS NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Los Balsos No.1", "LOS BALSOS NO. 1", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Los Balsos No.2", "LOS BALSOS NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("San Javier No.1", "SAN JAVIER NO. 1", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("San Javier No.2", "SAN JAVIER NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("San José la Cima No.2", "SAN JOSE LA CIMA NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Doce de Octubre No.1", "DOCE DE OCTUBRE NO. 1", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("Doce de Octubre No.2", "DOCE DE OCTUBRE NO. 2", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("B. Cerro  El Volador", "B. CERRO EL VOLADOR", med$NOMBRE_BAR, fixed = TRUE)
...
med$NOMBRE_BAR <- gsub("á", "a", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("é", "e", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("í", "i", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("ó", "o", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("ú", "u", med$NOMBRE_BAR, fixed = TRUE)
med$NOMBRE_BAR <- gsub("ñ", "n", med$NOMBRE_BAR, fixed = TRUE)
...
# Mayúsculas
med$NOMBRE_BAR  <- toupper(med$NOMBRE_BAR)
...
med <- med[med$OBJECTID!="9", ]
med <- med[med$OBJECTID!="159", ]
med <- med[med$OBJECTID!="286", ]
...
colnames(cluster_final)[colnames(cluster_final)=="BARRIO"] <- "NOMBRE_BAR"
...
med<-left_join(med,cluster_final,by="NOMBRE_BAR")
...
#pal <- colorFactor(c("cadetblue3", "cadetblue4", "khaki3", "snow4", #"lightsteelblue1"),domain = unique(med$Cl5G), n = 5)
...
# Poligonos, colores, leyenda



labels <- sprintf(
  
  # Se define la información a mostrar en el gráfico

  paste(
  "Barrio:",med$NOMBRE_BAR,"<br>",
  "Promedio anual accidentes:",med$promedio_anual_accidentes,"<br>",
  "Porcentaje accidentes fines de semana y festivo:", round(med$Porcentaje_accidentes_finsemyfestivos,4)*100,"%%","<br>",
  "Porcentaje de fatalidad:",round(med$Porcentaje_fatalidad,4)*100,"%%","<br>",
  "Porcentaje de atropello:",round(med$Porcentaje_atropello,4)*100,"%%")
...
# Se definen las propiedades de los polígonos
...
m %>% addPolygons(
  ...
  # Se definen las propiedades de la leyenda 
  
  labelOptions = labelOptions(
    style = list("font-weight" = "normal", padding = "3px 8px"),
    textsize = "15px",
    direction = "auto")) %>%
  addLegend(pal = pal, values = ~Cl5G, opacity = 0.7, title = NULL,
...
Se encuentra que los barrios con mayor promedio de accidentes por año están ubicados cerca al rio Medellín que es precisamente donde se encuentran las principales vías de la ciudad. El siguiente grupo con mayor promedio de accidentes se ubica un poco más hacia las laderas, sin embargo, este patrón es más visible hacia el sur que hacia el norte. Esto puede deberse a que los barrios del sur por lo general tienen mejores vías y mayor flujo vehicular.
Los barrios con menor accidentalidad se encuentran totalmente concentrados en las partes altas de las laderas, con una tendencia más clara hacia el norte. La mayoría de los barrios con menor accidentalidad se encuentran en dos sectores específicos de la ciudad: la comuna 13 y la comuna 1 (Popular).  
...
Grupo 2: Presenta el promedio mayor de accidentes por año en comparación con el resto del grupo, por lo que son a los que más se les debe prestar atención. Al estar ubicados muy cerca a la avenida regional, las vegas y demás vías céntricas de la ciudad, estos barrios deben de contar con presencia permanente del tránsito en horas pico para mejorar la movilidad. 
La accidentalidad no es tan alta los fines de semana comparada con los porcentajes de los otros grupos, por lo tanto, la mayor atención debe presentarse en las grandes concentraciones de vehículos que ocurren en los días laborales.  
